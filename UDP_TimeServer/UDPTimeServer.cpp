...
void main() 
{
	...
	// inet_ntoa (Internet address) is the reverse function (converts unsigned int to char *)5
	...
	while(true)
	{
		...
		// Check for client's request
		if(strcmp(recvBuff, "What's the time?") == 0)
		{ 
			GetTime(sendBuff);
		}
		else if(strcmp(recvBuff, "What's the time without date?") == 0)
		{
			GetTimeWithoutDate(sendBuff);
		}
		else if(strcmp(recvBuff, "What's the time since epoch?") == 0)
		{
			GetTimeSinceEpoch(sendBuff);
		}
		else if(strcmp(recvBuff, "Delay Estimation") == 0)
		{
		...
		else if(strcmp(recvBuff, "What's the year?") == 0)
		{
			GetYear(sendBuff);
		}
		else if(strcmp(recvBuff, "What's the day and month?") == 0)
		{
			GetMonthAndDay(sendBuff);
		}
		else if(strcmp(recvBuff, "What's the number of seconds since beg. of month?") == 0)
		{
...
void GetTimeWithoutDate(char *sendBuff)
{
	...
	char * times = asctime(ptm);    // string representing the time
	char * tokenized = strtok(times, " ");	// tokenizing the string
	for(int i = 0; i < 3; i++)   // extracting the time token
	{
		tokenized = strtok(NULL, " ");
	}

	strcpy(sendBuff, tokenized);
}
...
void GetTimeSinceEpoch(char *sendBuff)
{
	...
	ss << timer;
	string ts = ss.str();
	strcpy(sendBuff, ts.c_str());
}

void GetClientToServerDelayEstimation(char *sendBuff)
{
	stringstream ss;
	ss<<GetTickCount();		// using GetTickCount
	string ts = ss.str();
	strcpy(sendBuff, ts.c_str());
}
...
void MeasureRTT(char *sendBuff)
{
	strcpy(sendBuff, "request received!");  // server response
}
...
void GetMonthAndDay(char *sendBuff)
{
	...
	stringstream ss;
	char * day, *month, *dayOfTheMonth;
	day = strtok(timeStr, " ");
	month = strtok(NULL, " ");;
	dayOfTheMonth = strtok(NULL, " ");		
	ss << "Today is "<< day <<", "<< "the "<< dayOfTheMonth<<" of "<<month;
	string st = ss.str();
	strcpy(sendBuff, st.c_str());
}
...
void GetSecondsSinceBeginingOfMonth(char *sendBuff)
{
	// creating a new time format of seconds 
	// since epoch to beginning of month
	time_t now;
	time(&now);											
	tm * local = localtime(&now);
	tm older;       //this will hold time since epoch to beginning of month
	older.tm_mday = 1;
	older.tm_hour = 0;
	older.tm_sec = 0;
	older.tm_min = 0;
	older.tm_year = local->tm_year;
	older.tm_mon = local->tm_mon;
	double seconds = difftime(now, mktime(&older));
	stringstream ss;
	ss << seconds;
	string ts = ss.str();
	strcpy(sendBuff, ts.c_str());	
